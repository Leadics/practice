...
<table class="table table-striped table-responsive">
	...
	<?php foreach ( $NameArray as $key => $value) {
        if(!empty($value['Transaction']['id'])) { ?>
           <tr id="<?php echo 'list'.$key;?>" class='hidden'>
           		<td><strong><?php echo $key;?></strong></td>
              <td><?php echo $value['Transaction']['amount'];?></td>
              <?php if (!empty($value['Transaction']['perticular'])) {
                  echo '<td>Purchase</td>';
               } else  if (!empty($UserArray['id']) && $value['Transaction']['user_id'] ==$UserArray['id']){
                  echo '<td>Withdraw</td>';
                }?>
              <td><?php echo $value['Transaction']['binary'];?></td>
              <td><?php echo $value['Transaction']['referal'];?></td>
              <td></td>
           		<td><?php echo $value['Transaction']['created'];?></td>
              <?php if ($value['Transaction']['status'] == 1 ) {
