  ...
  <script src="<?php echo ABSOLUTE_URL;?>/js/go.js?n=<?php echo $n;?>"></script>
  ...
  <script id="code">
    function init() {
      ...
      var myNodeTemplate =
        $(go.Node, "Vertical",
          $(go.Picture,
            { margin: 10, width: 70, height: 50, background: "#DAE4E4" },
            ...
            toolTip:
              $(go.Adornment, "Auto",
                ...
                $(go.Panel, "Vertical",
                  ...
                  $(go.TextBlock, { margin: 3, width: 150  },
      ...
      myFullDiagram.addDiagramListener("InitialLayoutCompleted", function(e) {
        var node0 = myFullDiagram.findPartForKey("<?php echo $use['0']['email'];?>");
    ...
    function showLocalOnLocalClick() {
      ...
      if (selectedLocal !== null) {
        ...
        window.location = "<?php echo ABSOLUTE_URL?>/users/makeSession/"+selectedLocal.data.key;
    ...
    function setupDiagram(total) {
     ...
     <?php  foreach ($use as $key => $value) { ?>
        nodeDataArray.push({
          ...
          tool : "<?php echo 'Name : '.$value['name'].' \n Sponcer: '.$value['sponcer'].' \n  Amount : '.$value['donation'].'  \nLeft Business : '.$value['business']['left'].' \nRight Business : '.$value['business']['right'];?>"
        });
       <?php  } ?>
...
<div id="work" style="margin-bottom:-15px;">
  <div id="fullDiagram" style="height:0px;width:100%;border:1px solid black;margin:2px; background-color: #DAE4E4;"></div>
  <div id="localDiagram" style="height:700px;width:100%;border:0px solid black;margin:2px; background-color: #DAE4E4;"></div>  
</div>
</body>
